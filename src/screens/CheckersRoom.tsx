...
export interface CheckersRoomProps {
  code: string
  localName: string
  isHost: boolean
        ...
        <div style={{ flex: '1 1 320px', maxWidth: 460 }}>
          <div
            style={{
              ...
              background: '#b45309', color: '#fff', fontWeight: 700, fontSize: 15,
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {slots.map((seat, i) =>
              seat === null ? (
                ...
                >
                  <span className="avatar" style={{ background: '#b45309' }}>{(seat.name[0] ?? '?').toUpperCase()}</span>
                  <div>
                    <div style={{ fontWeight: 700, fontSize: 18 }}>
                      {seat.name}{seat.name === localName ? ' (you)' : ''}
                    </div>
                    <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                      {seat.isHost && <span className="chip" style={{ background: '#b45309', color: '#fff' }}>Host</span>}
