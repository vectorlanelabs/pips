...
export function BlackjackRoom({
  ...
  const slots = Array.from({ length: BLACKJACK_MAX_SEATS }, (_, i) => seats[i] ?? null)
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'clamp(18px,3vw,40px)' }}>
        <div style={{ flex: '1 1 380px', maxWidth: 460 }}>
          ...
          {isHost ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 22 }}>
              <button type="button" className="btn btn-lg" onClick={onAddHouseBot} disabled={seats.length >= BLACKJACK_MAX_SEATS}>
                Add house bot
              </button>
              <button type="button" className="btn btn-coral btn-lg" onClick={onStartGame} disabled={seats.length < BLACKJACK_MIN_SEATS}>
                Start game
              </button>
            </div>
          ) : (
          ...
          {seats.length < BLACKJACK_MAX_SEATS && (
        ...
        <div style={{ flex: '1 1 320px', maxWidth: 460 }}>
          ...
          <p style={{ marginTop: 16, fontSize: 15, color: 'var(--muted-text)' }}>
            $10–$500 bets, 1000 starting chips. Waiting for friends to type the code, or add a house bot.
          </p>
        </div>
      </div>
