...
export function SkipBoRoom({
  ...
  const slots = Array.from({ length: SKIPBO_MAX_SEATS }, (_, i) => seats[i] ?? null)
        ...
        <div style={{ flex: '1 1 380px', maxWidth: 460 }}>
          ...
          {isHost ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 22 }}>
              <button type="button" className="btn btn-lg" onClick={onAddHouseBot} disabled={seats.length >= SKIPBO_MAX_SEATS}>
                Add house bot
              </button>
              <button type="button" className="btn btn-coral btn-lg" onClick={onStartGame} disabled={seats.length < SKIPBO_MIN_SEATS}>
                Start game
              </button>
            </div>
          ) : (
