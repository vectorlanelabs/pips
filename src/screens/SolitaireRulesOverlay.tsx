...
const RULES: Record<SolitaireMode, { intro: string; bullets: string[] }> = {
  klondike: {
    ...
    bullets: [
      ...
      'Stock: click to turn one card onto the waste. When the stock runs out, click it again to flip the waste back over. Take as many passes as you like.',
      'Click a card, then click where it goes, or drag it there. Click a selected card again to send it to its foundation. Cards can come back off a foundation if you need them.',
      'Auto-play lights up once every card is face up and the stock and waste are empty. It plays every remaining legal foundation move for you.',
      'Undo is unlimited. Deal again starts a fresh shuffle.',
    ],
  },
  klondike3: {
    intro: 'Klondike with a tighter stock. Draw three at a time, and only the last one drawn is in play.',
    bullets: [
      'Deal: seven columns, one to seven cards, only the top card face up. The rest is the stock.',
      'Stock: click to turn up to three cards onto the waste at once. Only the top (most recently drawn) card is playable. Draw again to dig through the other two. When the stock runs out, click it again to flip the waste back over.',
      'Tableau: stack cards in descending order, alternating red and black. Move any face-up run as a unit.',
      'Only a King (or a run starting with one) can move into an empty column.',
      'Click a card, then click where it goes, or drag it there. Click a selected card again to send it to its foundation. Cards can come back off a foundation if you need them.',
      'Auto-play lights up once every card is face up and the stock and waste are empty. It plays every remaining legal foundation move for you.',
  ...
  freecell: {
    intro: 'Every card is face up from the start.',
    bullets: [
      ...
      'Moving a run at once is a shortcut for moving it card by card through the free cells. You can move at most (empty cells + 1) × 2^(empty columns) cards, and an empty column you\'re moving INTO doesn\'t count.',
      'Click a card, then click where it goes, or drag it there. Click a selected card again to send it to its foundation.',
      'Auto-play lights up once there’s a legal foundation move to make. FreeCell deals every card face up, so it’s often available early.',
      'Undo is unlimited. Deal again starts a fresh shuffle.',
    ],
  },
  spider: {
    intro: 'Ten columns, two suits. Build eight complete King-to-Ace runs.',
    bullets: [
      ...
      'Click a card, then click where it goes, or drag it there. There\'s no foundation shortcut or Auto-play here since completed runs clear on their own.',
  ...
  pyramid: {
    ...
    bullets: [
      ...
      'A card is exposed once both cards it rests on (the two directly below it) are gone. The base row is always exposed.',
      'Remove any two exposed cards whose ranks add up to 13 (A=1, J=11, Q=12). A King is worth 13 on its own, so it clears alone.',
      'The waste pile\'s top card counts as exposed too, so it can pair with an exposed pyramid card.',
      'Stock: click to turn one card onto the waste. When the stock runs out, click it again to flip the waste back over. Take as many passes as you like.',
      'Click a card, then click one that adds up to 13 with it, or drag it there. Click a selected King again to clear it.',
      'Undo is unlimited. Deal again starts a fresh shuffle.',
    ],
  },
}
